...
public class PromptBuilder {
     ...
     * The first phase already requested CHECK and paused. This second pass may
     * persist at most one immediate mechanical consequence (for example HP
     * loss after a fall) before narrating the resolved outcome.
     */
    public String buildDiceContinuationDirectorPrompt(
            MemoryContext memoryContext,
            DirectorPromptState directorState,
            CampaignPromptState campaignState
    ) {
        ...
        prompt.append("\n\nSTATE AFTER ROLL (REFERENCE ONLY):");
        appendStateLine(prompt, "LOCATION", state.getLocation());
        appendStateLine(prompt, "HP", state.getHealth());
        appendStateList(prompt, "NPCS", state.getActiveNpcs());
        appendStateList(prompt, "EFFECTS", state.getEffects());

        prompt.append("\n\nTASK:\nЗафиксируй непосредственное механическое последствие уже выполненной проверки, затем расскажи её итог.");
        return prompt.toString();
    }
